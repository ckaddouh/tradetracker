        ...
        <div v-for="watch in earningsStore.upcoming" :key="watch._id" class="earnings-card">
          ...
          <div class="post-earnings">
            ...
            <div class="stat-pill">
              ...
              <button
                v-if="watch.reportDate && new Date(watch.reportDate) < new Date()"
...
async function autoFillResults() {
  ...
  const dueWatches = earningsStore.watches.filter(w => {
    ...
    return reportDate <= now || reportDate.toDateString() === today
  })
  ...
  for (const watch of dueWatches) {
    try {
      ...
      console.log(`Could not auto-fill results for ${watch.ticker}:`, err.message)
    }
  }
}
